...
//  Edited by Saaleh Poovathumkadavil on 8/20/23
//
...
public struct Split<P: View, D: View, S: View>: View {
    ...
    /// Public init overload only allows `primary` and `secondary` and `fraction`, with `splitter` defaulting to Splitter.
    ///
    /// The `layout`, `fraction`,  `hide` ,  `styling`, `constraints`, and any custom `splitter` must be specified using the modifiers if they are not defaults
    public init(@ViewBuilder primary: @escaping ()->P, @ViewBuilder secondary: @escaping ()->S, fraction: CGFloat) where D == Splitter {
        let layout = LayoutHolder()
        let fraction = FractionHolder()
        let hide = SideHolder()
        let styling = SplitStyling()
